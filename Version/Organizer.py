...
class App(tkinter.Tk):
    def __init__(self):
        ...
        botton_colors=[{"bg":"#ede2e1","fg":"#000000"}]
...
#       fixed button
        ...
        self.undo_button=tkinter.Button(self,text="undo",width=13,height=1,bg=botton_colors[0]["bg"],fg=botton_colors[0]["fg"],command=self.undo)
...
#       configuraciones de la GUI
    ...
    def undo(self):
        if self.previousMove=={}:
            ...
            src=r"{path}/{name}".format(path=self.previousMove['path'],name=self.previousMove['img_name'])
            dst=r"{}".format(self.current_path)
            shmove(src,dst)
            self.move_img.remove(self.previousMove['img_name'])
            self.previousMove={}
            self.previous()
    def next(self):
    ...
    def move(self,pathname:string):
        ...
        self.previousMove={"img_name":self.file_list[self.current_img],"path":dst}
        self.next()
    ...
    def add_button(self,name: string):
        margin=100
